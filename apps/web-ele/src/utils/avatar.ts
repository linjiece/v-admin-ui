 ...
 * 判断是否是汉字
 */
function isChinese(ch: string, strict = false): boolean {
  if (ch.length !== 1) return false;
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  const code = ch.codePointAt(0)!;
  // 基本汉字区
  if (code >= 0x4e_00 && code <= 0x9f_ff) return true;
  if (strict) return false;
  // 扩展 A-F
  return (
    (code >= 0x34_00 && code <= 0x4D_BF) || // Ext-A
    (code >= 0x2_00_00 && code <= 0x2_eb_ff) || // Ext-B/C/D/E/F
    false
  );
}
