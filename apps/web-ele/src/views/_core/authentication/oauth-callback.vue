...
<template>
  <div class="flex min-h-screen items-center justify-center">
    <div class="min-w-80 rounded-xl bg-background p-12 text-center">
      <div
        v-if="loading"
      ...
      >
        ...
        <div class="relative flex size-20 items-center justify-center">
          <!-- 外层扩散波纹 -->
          <div
            class="absolute h-full w-full animate-ping rounded-full bg-primary/20 opacity-75"
          ></div>
          <!-- 中层静态光晕 -->
          <div class="absolute size-14 rounded-full bg-primary/10"></div>
          <!-- 内层旋转圆环 -->
          <div
            class="size-10 animate-spin rounded-full border-4 border-primary border-t-transparent"
          ></div>
        </div>

        <p class="mt-6 animate-pulse text-sm font-medium text-muted-foreground">
          正在处理登录...
        </p>
      </div>
      ...
      <div v-else class="flex flex-col items-center gap-5">
        ...
        <p class="m-0 text-base text-destructive">{{ errorMessage }}</p>
        <button
          class="cursor-pointer rounded-md border-none bg-primary px-6 py-2.5 text-sm text-primary-foreground transition-colors hover:bg-primary/90"
          @click="backToLogin"
        >
          返回登录
        </button>
